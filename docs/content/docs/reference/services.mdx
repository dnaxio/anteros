...
Services support the same `api.access` rules as collections. Use wildcard `*` for a default policy and specific action keys for fine-grained control.

When no `api.access` is defined, all authenticated users can call any action.
...
export default define.Service({
  ...
  api: {
    access: {
      ...
      'health': true,             // health is public (to any authenticated user)
    }
  },
  actions: {
    stats: async ({ rest }) => { ... },
...
| Rule | Behavior |
|------|----------|
| No `api.access` | All authenticated users can call any action |
| `'*': false` | Deny all, allow only explicitly allowed actions |
| `'*': true` | Allow all, deny only explicitly denied actions |
| `action: true` | Allow this specific action |
| `action: false` | Deny this specific action |
| `action: (ctx) => boolean` | Custom function, receives `{ rest, error, jwt, token }` |
