...
const cipher = utils.crypt.resolve(); // même mode / clés que server.encryption
const ssn = await cipher.decrypt(doc.ssn, true); // champ chiffré → valeur d'origine

// json avec sous-chemins chiffrés → passe les champs en 3e paramètre
const address = await cipher.decrypt(doc.address, true, ['zip']);
// → { street: 'Rue X', city: 'Paris', zip: '75000' } (zip décrypté, le reste tel quel)
```
