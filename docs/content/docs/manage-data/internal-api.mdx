...
### findStream / findPages / aggregateStream
...
// Paginated batches: { count, docs, hasNext() }
...
|--------|---------|-------|
| `findStream(collection, params, options?)` | `AsyncGenerator` | Same query language as `find`, **no default `$limit`** (streams everything) |
| `findPages(collection, params, options?)` | `AsyncGenerator` | Batched: yields `{ count, docs, hasNext() }` — total + up to `batchSize` docs per page |
| `aggregateStream(collection, pipeline, options?)` | `AsyncGenerator` | Raw pipeline, same safety checks as `aggregate` |

All accept a third `options` parameter:
...
| `withCount` | `boolean` | Runs a background count — **`findStream`/`aggregateStream`**: opt-in (default `false`); **`findPages`**: default `true`, set `false` to skip the count query (pages then have no `count` field). `findStream` counts docs matching `$match`; `aggregateStream` counts the pipeline **output** (via `$count`) — each yielded item becomes `{ count, doc }` |
