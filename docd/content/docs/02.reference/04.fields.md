---
title: Fields
description: Field types and options for collection schemas in anteros Framework.
navigation:
  icon: lucide:columns-3
---

Fields define the shape and validation rules of each document in a collection. Every field has a `name` and a `type`, plus optional configuration that controls validation, indexing, default values, and more.

## Basic Usage

```typescript
import { define } from "@anteros/core";

export default define.Collection({
  slug: "users",
  fields: [
    { name: "name", type: "string", required: true },
    { name: "email", type: "email", required: true, unique: true },
    { name: "age", type: "integer" },
    { name: "isActive", type: "boolean", defaultValue: true },
  ],
});
```

## Field Types

| Type                 | Validation                             | Description                                        |
| -------------------- | -------------------------------------- | -------------------------------------------------- |
| `string`             | `Joi.string()`                         | Plain text value                                   |
| `number`             | `Joi.number()`                         | Floating-point number                              |
| `integer`            | `Joi.number().integer()`               | Integer number                                     |
| `boolean`            | `Joi.boolean()`                        | `true` or `false`                                  |
| `email`              | `Joi.string().email()`                 | Valid email address                                |
| `password`           | `Joi.string()`                         | Automatically hashed with Bun on insert and update |
| `uuid`               | `Joi.string().uuid()`                  | UUID string                                        |
| `url`                | `Joi.string().uri()`                   | Valid URL                                          |
| `date`               | `Joi.date()`                           | Date value                                         |
| `datetime-local`     | `Joi.date()`                           | DateTime value                                     |
| `enum`               | `Joi.string().valid(...)`              | Value from a predefined list                       |
| `array`              | `Joi.array()`                          | Array of values                                    |
| `json`               | `Joi.object()`                         | Arbitrary JSON object                              |
| `random`             | `Joi.string()` or `Joi.number()`       | Auto-generated value on insert                     |
| `relationship`       | `Joi.string()` (ObjectId)              | Reference to another collection                    |
| `ipv4`               | `Joi.string().ip({ version: 'ipv4' })` | IPv4 address                                       |
| `ipv6`               | `Joi.string().ip({ version: 'ipv6' })` | IPv6 address                                       |
| `geojson.Point`      | —                                      | GeoJSON Point                                      |
| `geojson.LineString` | —                                      | GeoJSON LineString                                 |
| `geojson.Polygon`    | —                                      | GeoJSON Polygon                                    |

## Field Options

```typescript
{
  name: string;
  type: FieldType;
  description?: string;
  required?: boolean;
  unique?: boolean | 1 | 0;   // Mongo-style: true/1 → unique index, false/0 → non-unique
  nullable?: boolean;
  defaultValue?: any;
  validate?: {
    schema: Joi.Schema;
  };
  index?: boolean;
  indexType?: 'text' | 'hashed' | '2dsphere' | '2d';
  indexOptions?: {
    expireAfterSeconds?: number;
    sparse?: boolean | 1 | 0;
    unique?: boolean | 1 | 0;
  };
  enumOptions?: { ... };
  relation?: { ... };
  randomOptions?: { ... };
}
```

| Option              | Type                           | Description                                                                                                                                                                          |
| ------------------- | ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `name`              | `string`                       | Field name (required)                                                                                                                                                                |
| `type`              | `FieldType`                    | Field type (required)                                                                                                                                                                |
| `description`       | `string`                       | Human-readable description                                                                                                                                                           |
| `required`          | `boolean`                      | Reject the document if this field is missing                                                                                                                                         |
| `unique`            | `boolean \| 1 \| 0`            | Create a unique MongoDB index (`true`/`1` = unique, `false`/`0` = not)                                                                                                               |
| `nullable`          | `boolean`                      | Allow `null` as a value                                                                                                                                                              |
| `defaultValue`      | `any`                          | Value applied automatically on insert when absent                                                                                                                                    |
| `validate`          | `{ schema: Joi.Schema }`       | Custom Joi validation schema (overrides the default)                                                                                                                                 |
| `index`             | `boolean`                      | Create a MongoDB index on this field                                                                                                                                                 |
| `indexType`         | `string`                       | Index type: `text`, `hashed`, `2dsphere`, or `2d`                                                                                                                                    |
| `indexOptions`      | `object`                       | Additional MongoDB index options — `unique`/`sparse` are coerced to real booleans (`1`→`true`, `0`→`false`) and `null`/`undefined` values are dropped (Mongo rejects `sparse: null`) |
| `encryption`        | `boolean`                      | Encrypt this field's values at rest — uses `server.encryption` (mode + keys). Encrypted on insert/update, decrypted manually on reads                                                |
| `encryptionOptions` | `{ path: string \| string[] }` | Encrypt only sub-path(s) inside a `json` / array-of-json field (dot notation) — e.g. `{ path: 'zip' }` on `address` encrypts `address.zip`                                           |

## Field Encryption

Set `encryption: true` on a field to store its values encrypted at rest, or `encryptionOptions: { path }` to encrypt only a sub-path inside a json field:

```typescript
{ name: 'ssn', type: 'string', encryption: true }         // whole value encrypted
{ name: 'profile', type: 'json', encryption: true }        // whole json object encrypted
{ name: 'address', type: 'json', encryptionOptions: { path: 'zip' } }        // only address.zip
{ name: 'contacts', type: 'json', encryptionOptions: { path: ['phone', 'email'] } } // multiple paths
{ name: 'addresses', type: 'json', encryptionOptions: { path: 'zip' } }      // array of json → each element's zip
```

Requirements and behavior:

- **Server config required** — `server.encryption` (mode + keys) must be configured; otherwise writes throw `ENCRYPTION_NOT_CONFIGURED`.
- **Mode-driven** — encryption uses `utils.crypt.resolve()` (symmetric AES-256-GCM or asymmetric RSA-OAEP envelope, per `server.encryption.mode`).
- **Writes**: `insertOne`, `insertMany`, `updateOne`/`updateMany`/`findOneAndUpdate` (`$set`/`$setOnInsert`), `bulkWrite`, `bulkUpdate` — the field value is encrypted before storage. API responses return the **plaintext**.
- **Reads: NO automatic decryption** — `find`, `findOne`, `aggregate`, streams return the **ciphertext** as stored. Decrypt explicitly when needed:

```typescript
import { utils } from "@anteros/core";

const cipher = utils.crypt.resolve(); // same mode / keys as server.encryption
const ssn = await cipher.decrypt(doc.ssn, true); // encrypted field → original value

// json with encrypted sub-paths → pass the fields as the 3rd parameter
const address = await cipher.decrypt(doc.address, true, ["zip"]);
// → { street: 'Rue X', city: 'Paris', zip: '75000' } (zip decrypted, the rest untouched)
```

- **Round-trips any JSON value** (string, number, object, array) — internally `JSON.stringify` + `encrypt`, decrypted with `parseJson`.
- ⚠️ **Opaque in Mongo** — encrypted fields store ciphertext: don't use them in `$match`/`$sort`/`$group` (they'd match ciphertext). Operators like `$inc`/`$push`/`$addToSet` on an encrypted field are rejected (`ENCRYPTED_FIELD_OPERATOR`); only `$set`/`$setOnInsert` can write them. Don't mark `relationship` fields (breaks `$include`).

## Custom Validation

Each field gets a default Joi schema based on its `type`. To override it with your own rules, use the `validate` option. Import `v` from `@anteros/core` — it is a Joi instance.

```typescript
import { define, v } from "@anteros/core";

export default define.Collection({
  slug: "users",
  fields: [
    {
      name: "age",
      type: "number",
      validate: {
        schema: v.number().min(0).max(120),
      },
    },
  ],
});
```

When `validate.schema` is provided, it **replaces** the auto-generated schema for that field. The `required` option still applies on top of it.

### Examples

String with length constraints:

```typescript
{
  name: 'username',
  type: 'string',
  required: true,
  validate: {
    schema: v.string().alphanum().min(3).max(30),
  },
}
```

Email restricted to a domain:

```typescript
{
  name: 'email',
  type: 'email',
  validate: {
    schema: v.string().email().pattern(/@acme\.com$/),
  },
}
```

Number within a range:

```typescript
{
  name: 'rating',
  type: 'number',
  validate: {
    schema: v.number().min(1).max(5),
  },
}
```

Array with item validation:

```typescript
{
  name: 'scores',
  type: 'array',
  validate: {
    schema: v.array().items(v.number().min(0).max(100)).max(10),
  },
}
```

JSON object with specific shape:

```typescript
{
  name: 'address',
  type: 'json',
  validate: {
    schema: v.object({
      street: v.string().required(),
      city: v.string().required(),
      zip: v.string().pattern(/^\d{5}$/).required(),
    }),
  },
}
```

::prose-callout{variant="tip"}

`v` is a full [Joi](https://joi.dev/api/) instance. Any Joi method (`v.string()`, `v.number()`, `v.object()`, `v.array()`, `v.alternatives()`, etc.) is available.

::

## Password

Fields with `type: 'password'` are automatically hashed using `Bun.password.hashSync()` on every **insert** and **update** operation. You never store or receive a plain-text password.

```typescript
{ name: 'password', type: 'password', required: true }
```

::prose-callout{variant="tip"}

Combine with `privateFields` in the collection API config to hide the hash from REST responses.

::

```typescript
api: {
  privateFields: [/password/],
}
```

## Enum

Use `enumOptions` to restrict a field to a set of allowed values.

### Single value

```typescript
{
  name: 'role',
  type: 'enum',
  enumOptions: {
    items: ['admin', 'editor', 'viewer'],
  },
}
```

### Multiple values

When `multiple` is `true`, the field accepts an **array** of values from the list.

```typescript
{
  name: 'tags',
  type: 'enum',
  enumOptions: {
    multiple: true,
    items: ['javascript', 'typescript', 'rust', 'go'],
  },
}
```

## Relationship

Reference documents from another collection. On query, use `$include` to populate the related data with a `$lookup`.

### One-to-one

```typescript
{
  name: 'author',
  type: 'relationship',
  relation: {
    to: 'users',
  },
}
```

### One-to-many

```typescript
{
  name: 'contributors',
  type: 'relationship',
  relation: {
    to: 'users',
    hasMany: true,
  },
}
```

### Custom pipeline

Attach an aggregation pipeline to the `$lookup` stage:

```typescript
{
  name: 'author',
  type: 'relationship',
  relation: {
    to: 'users',
    pipeline: [
      { $project: { name: 1, email: 1 } },
    ],
  },
}
```

::prose-callout{variant="info"}

When inserting or updating, you can pass either a full object with `_id` or just the ObjectId string — anteros extracts the id automatically.

::

## Random

Auto-generate a unique value on **insert only**. The value is not regenerated on update.

```typescript
{
  name: 'code',
  type: 'random',
  randomOptions: {
    length: 10,
    startWith: 'BL-',
  },
}
```

`startWith` and `endWith` can also be **functions** that receive the generated value and return a customized prefix/suffix:

```typescript
{
  name: 'ref',
  type: 'random',
  randomOptions: {
    length: 8,
    useNumbers: true,
    startWith: (value) => 'BP-' + value,   // → "BP-38472910"
    endWith:   (value) => value + '-X',     // → "BP-38472910-X"
  },
}
```

### Random Options

| Option           | Type                                    | Default | Description                                        |
| ---------------- | --------------------------------------- | ------- | -------------------------------------------------- |
| `length`         | `number`                                | —       | Number of random characters to generate (required) |
| `useLetters`     | `boolean`                               | `false` | Include letters (`a-zA-Z`)                         |
| `useNumbers`     | `boolean`                               | `true`  | Include digits (`0-9`)                             |
| `includeSymbols` | `string`                                | `''`    | Extra characters to include in the charset         |
| `excludeSymbols` | `string`                                | `''`    | Characters to exclude from the charset             |
| `startWith`      | `string \| ((value: string) => string)` | `''`    | Prefix (static string or function)                 |
| `endWith`        | `string \| ((value: string) => string)` | `''`    | Suffix (static string or function)                 |
| `toLowerCase`    | `boolean`                               | `false` | Convert to lowercase                               |
| `toUpperCase`    | `boolean`                               | `false` | Convert to uppercase                               |
| `toNumber`       | `boolean`                               | `false` | Parse the result as an integer                     |

::prose-callout{variant="info"}

When `unique` is set or `randomOptions` is defined, anteros automatically checks the database for duplicates and regenerates until a unique value is found.

::

### Examples

Numeric order code:

```typescript
{
  name: 'orderNumber',
  type: 'random',
  randomOptions: {
    length: 8,
    useNumbers: true,
    startWith: 'ORD-',
  },
}
```

Alphanumeric token:

```typescript
{
  name: 'inviteCode',
  type: 'random',
  randomOptions: {
    length: 12,
    useLetters: true,
    useNumbers: true,
    toUpperCase: true,
  },
}
```

## Indexing

Control MongoDB indexes directly from the field definition.

```typescript
{ name: 'email', type: 'email', unique: true }
```

Setting `unique: true` automatically creates a unique index. For more control, use `index`, `indexType`, and `indexOptions`:

```typescript
{
  name: 'location',
  type: 'geojson.Point',
  index: true,
  indexType: '2dsphere',
}
```

### TTL Index

Expire documents automatically:

```typescript
{
  name: 'sessionExpires',
  type: 'date',
  index: true,
  indexOptions: {
    expireAfterSeconds: 3600,
  },
}
```

## Default Values

The `defaultValue` is applied on **insert** when the field is absent from the payload.

```typescript
{ name: 'role', type: 'string', defaultValue: 'member' }
{ name: 'isActive', type: 'boolean', defaultValue: true }
{ name: 'views', type: 'number', defaultValue: 0 }
```

::prose-callout{variant="info"}

`createdAt` and `updatedAt` are managed automatically by anteros — you don't need to declare them. `createdAt` is set on insert, and `updatedAt` is refreshed on every insert and update.

::

## Full Example

```typescript
import { define } from "@anteros/core";

export default define.Collection({
  slug: "products",
  fields: [
    { name: "name", type: "string", required: true },
    {
      name: "sku",
      type: "random",
      randomOptions: { length: 8, startWith: "SKU-", useNumbers: true },
    },
    { name: "price", type: "number", required: true },
    { name: "description", type: "string" },
    { name: "email", type: "email", unique: true },
    {
      name: "category",
      type: "enum",
      enumOptions: { items: ["electronics", "clothing", "food"] },
    },
    {
      name: "tags",
      type: "enum",
      enumOptions: { multiple: true, items: ["new", "sale", "featured"] },
    },
    { name: "stock", type: "integer", defaultValue: 0 },
    { name: "isActive", type: "boolean", defaultValue: true },
    { name: "metadata", type: "json" },
    { name: "images", type: "array" },
    { name: "brand", type: "relationship", relation: { to: "brands" } },
    {
      name: "relatedProducts",
      type: "relationship",
      relation: { to: "products", hasMany: true },
    },
    { name: "website", type: "url" },
    {
      name: "location",
      type: "geojson.Point",
      index: true,
      indexType: "2dsphere",
    },
  ],
});
```
