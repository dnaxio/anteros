...
```
POST /api/:tenant_id/services/:service/:action
```
...
Services are invoked with `POST` requests on the `/api/:tenant_id/services/:service/:action` endpoint. The payload to the action is read from the `data` field of the JSON body:

```http
POST /api/v1/services/factures/generate
...
v1/
└── services/
    └── factures.service.ts    → POST /api/v1/services/factures/:action

v2/
└── services/
    └── factures.service.ts    → POST /api/v2/services/factures/:action
```

A service registered in `v1` is **not** reachable through `/api/v2/services/...` even if the file content is identical.

## Activity Logging

Every service action call is automatically logged as an activity — once, whether it comes from the HTTP route or from `rest.runService()` directly. Each call records:

| Field          | Description                                                                        |
| -------------- | ---------------------------------------------------------------------------------- |
| `tenant`       | The tenant ID                                                                      |
| `action`       | `"runService"`                                                                     |
| `collection`   | The service name                                                                   |
| `input.action` | The service action name being executed, e.g. `charge`                              |
| `input.data`   | The `data` payload                                                                 |
| `result`       | Always `null` — the response is never stored                                       |
| `error`        | Error details if the action failed                                                 |
| `status`       | `"success"` or `"error"`                                                           |
| `internal`     | `true` for a server-side call, `false` through `POST /api/:tenant_id/services/...` |
| `duration`     | Execution time in milliseconds                                                     |
| `token`        | The decoded JWT token used for authentication                                      |

Activities are stored in the `_audit_` collection of the tenant's database.
...
| Endpoint     | `POST /api/:tenant_id/services/:service/:action`               |
