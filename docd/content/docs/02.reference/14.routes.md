...
The prefix is **yours to choose** — and since tenant routes are mounted _before_ the framework API, one that starts with a reserved family (`/v1/vars/…`, `/v1/collections/…`) would shadow it. The boot log warns about it; use another name, or mount under the tenant's own namespace (e.g. `prefix: '/api/v1'`, which is what the scaffold uses).
