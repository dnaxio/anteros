...
- **Simplicity** — One namespace (`POST /api/:tenant_id/…`) with a family per surface — `collections/:collection/:action`, `services/…`, `vars`, `upload`, `files`, `agents`, `mcp`. No boilerplate.
...
define.Collection({
  ...
  fields: [
    ...
    { name: "price", type: "number", required: true },
    {
      name: "sku",
      type: "random",
      randomOptions: { length: 8, startWith: "SKU-" },
    },
  ],
});
```
